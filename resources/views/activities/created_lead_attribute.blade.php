<tr id="activity-{{$activity->id}}" class="main-row {{$loop->last?'remove-border-bottom':''}}">
    ...
    <td class="flex items-center mr-2 float-right md:mr-0 md:float-none">
        <div class="hidden md:block">
            <p class="text-gray-400">Added attribute:
                <span class="text-black">{{$activity->subject->attribute->name}}</span>
            </p>
            <p class="text-gray-400">with value:
                <span class="text-black">{{$activity->changes['after']['value']}}</span>
            </p>
        </div>
